...
void setUp(void) {
  Serial.println(">>> SETUP: Attaching motor B...");
  testMotor.setPeriodHertz(MOTOR_PWM_FREQUENCY);
  testMotor.attach(UPPER_RIGHT_MOTOR, MIN_PULSE_LENGTH, MAX_PULSE_LENGTH);
  Serial.println(">>> Motor B attached!");
}
...
void tearDown(void) {
  ...
  delay(500);
  testMotor.detach();
  Serial.println(">>> Motor B detached!");
}

void test_arm_and_run_motor_b(void) {
...
void setup() {
  ...
  Serial.println("\n\n>>> MOTOR B TEST STARTING <<<");
  
  UNITY_BEGIN();
  RUN_TEST(test_arm_and_run_motor_b);
  UNITY_END();
