...
void setUp(void) {
  Serial.println(">>> SETUP: Attaching motor A...");
  testMotor.setPeriodHertz(MOTOR_PWM_FREQUENCY);
  testMotor.attach(UPPER_LEFT_MOTOR, MIN_PULSE_LENGTH, MAX_PULSE_LENGTH);
  Serial.println(">>> Motor A attached!");
}
...
void tearDown(void) {
  ...
  delay(500);
  testMotor.detach();
  Serial.println(">>> Motor A detached!");
}

void test_arm_and_run_motor_a(void) {
...
void setup() {
  ...
  Serial.println("\n\n>>> MOTOR A TEST STARTING <<<");
  
  UNITY_BEGIN();
  RUN_TEST(test_arm_and_run_motor_a);
  UNITY_END();
